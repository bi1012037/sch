...
try:
    ...
    in_index = 0
mail_check = 0
#scritpt = """curl -X POST -H "Content-Type: application/json" -d '{"value1":"日記簿有訪客進入，請回來查看！","value2":"","value3":""}' https://maker.ifttt.com/trigger/Alert/with/key/bSH9xED1ZjuA-M74D-cDdk > log.txt"""
ifttt_url = "https://maker.ifttt.com/trigger/Alert/with/key/bSH9xED1ZjuA-M74D-cDdk"
headers = {
    "Content-Type": "application/json"
}
data = {
    "value1": "日記簿有訪客進入，請回來查看！",
}
if(ip!="125.228.205.164"):  #125.228.205.164
    ...
    if "%s_search(%s" % (ip,datetime.datetime.now().strftime("%Y-%m-%d %H:")) not in vc:
        #mail_check += 1
        #os.system('ssh linchingyen@125.228.205.164 "python E:\\sch\\run.py"')
        ...
        for i in range(0,1):
            ...
            #response = requests.post(ifttt_url, headers=headers, json=data)
            requests.get("https://author.lugeshop.com/discord.php?message=%s&channel=note" % data["value1"])
    if "%s_search(%s" % (ip,datetime.datetime.now().strftime("%Y-%m-%d %H:%M")) not in vc:
